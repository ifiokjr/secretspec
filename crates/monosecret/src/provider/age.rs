...
fn provider_err(msg: impl Into<String>) -> MonosecretError {
	MonosecretError::ProviderOperationFailed(msg.into())
}
...
impl ParsedIdentity {
	fn to_recipients(&self) -> Result<Vec<Box<dyn Recipient + Send>>> {
		match self {
			Self::Age(identity_file) => {
				identity_file.to_recipients().map_err(|e| {
					provider_err(format!(
	...
	fn into_identities(self) -> Result<Vec<Box<dyn Identity + Send + Sync>>> {
		match self {
			Self::Age(identity_file) => {
				identity_file.into_identities().map_err(|e| {
					provider_err(format!(
